...
const LLM_MODELS: Record<string, {model: string, name: string, apiKey: string, provider: string}> = {
    'gemini-default': {
        model: 'models/gemini-2.5-flash',
...
let chat: any = chatModel.startChat();
        ...
        try {
            ...
            if (currentModelConfig.provider === 'google') {
                ...
                if (functionCalls && functionCalls.length > 0) {
                    ...
                    // Perform the search
                    const searchResults = await performGoogleSearch((functionCalls[0].args as any).query);
